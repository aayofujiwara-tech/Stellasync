...
export default function ManagerLayout() {
  ...
  useEffect(() => {
    if (!loading) {
      ...
      else if (role !== 'area_manager') navigate('/cast/home', { replace: true })
    }
  }, [user, loading, role, navigate])

  if (loading || !user || role !== 'area_manager') {
    return (
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#0F0F14' }}>
      ...
      >
        <div className="flex items-center gap-2">
          <LayoutDashboard size={18} style={{ color: '#7C6FE0' }} />
          <h1 className="text-base font-bold" style={{ color: '#7C6FE0' }}>Stellasync</h1>
          <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: '#1A2A1A', color: '#80C080' }}>
            MANAGER
          </span>
        </div>
